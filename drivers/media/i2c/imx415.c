...
static int imx415_s_ctrl(struct v4l2_ctrl *ctrl)
{
	...
	case V4L2_CID_DIGITAL_GAIN:
		...
		default:
		dev_err(sensor->dev, "unsupported control id: 0x%x\n", ctrl->id);
		ret = -EINVAL;
		break;
	}
...
static int imx415_ctrls_init(struct imx415 *sensor)
{
	...
	v4l2_ctrl_new_std(&sensor->ctrls, &imx415_ctrl_ops,
			  ...
			  IMX415_AGAIN_MIN);
	/* Stub for libcamhal; no HW digital gain - s_ctrl() ignores writes */
	v4l2_ctrl_new_std(&sensor->ctrls, &imx415_ctrl_ops,
			  V4L2_CID_DIGITAL_GAIN, 1, 1, 1, 1);
...
static int imx415_parse_hw_config(struct imx415 *sensor)
{
	...
	ep = fwnode_graph_get_endpoint_by_id(dev_fwnode(sensor->dev), 0, 0, FWNODE_GRAPH_ENDPOINT_NEXT);
	if (!ep)
		return -ENXIO;
...
static const struct acpi_device_id imx415_acpi_ids[] = {
        {"IMXSN415"},
        {}
};

MODULE_DEVICE_TABLE(acpi, imx415_acpi_ids);
...
static struct i2c_driver imx415_driver = {
	...
	.driver = {
		...
		.acpi_match_table = ACPI_PTR(imx415_acpi_ids),
